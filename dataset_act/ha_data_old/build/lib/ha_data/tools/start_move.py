...
def get_start_index(path_list, naming=''):
    arm_list = []
    total_frame_num = 0

    thresh_hold = 0.0025
    offset = 5 # start_idx前移
    ...
    for season_folder in path_list:
        ...
        if naming == '':
            ...
            exit()
        arm_key = naming.ROBOT_PREFIX + "right_arm/joint_angle"
        ...
        for hdf5_p in j_data["url"]:
            ...
            with h5py.File(h5_path, "r") as root:
                seq_length = root[arm_key].shape[0]
                total_frame_num += seq_length
                try:
                    for idx in range(seq_length-1):
                        dif = np.sum(np.abs(root[arm_key][idx+1] - root[arm_key][idx]))
                        # print(dif)
                        if dif > thresh_hold:
...
def get_max_img_length():
    path_list = [
        "/mnt/yrfs/Team/Robot/train/r0_han_water/1001/acc_A-A/",
    ]
    hdf5_list = []
    camera_name_list = [
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_list", "-p", type=str, default="path_list.txt")
    ...
    get_start_index(path_list, NamingRule2412())
    # get_max_img_length()
